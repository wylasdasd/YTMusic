...
| `Services/Playback/IPlaybackInstance.cs` | 实例生命周期：`Attach` / `Detach` / Play / Pause / Seek |
| `Services/Playback/IPlaybackHost.cs` | 宿主回调：代理、Web 同步、原生服务访问 |
| `Services/Playback/PlaybackInstances.cs` | 五种 `IPlaybackInstance` 实现 |
| `Services/INativeAudioPlaybackService.cs` | 原生音频抽象 |
| `Services/INativeVideoPlaybackService.cs` | 原生视频抽象（含 `companionAudioUrl`、`autoPlay`） |
...
| `Hybrid` | 原生音频轨 + Web `<video>` 画面（静音） | Windows 在线分离流（video-only + audio-only） |
| `None` | 无活跃实例 | 停止 / 失败清空后 |
...
| Windows | 是 | 分离流 | `Hybrid`（原生音频 + Web 视频画面） |
| Windows | 是 | muxed / 单 video URL | `WebMuxedVideo` |
